             ...
             const double & size )
{
    ...
    return CArea( nearest_gpost, 0.06 + size );
}
}
...
MPObject::_inc()
{
    ...
    points.push_back(PVector(ServerParam::instance().goalPostRadius()+(ServerParam::PITCH_LENGTH*0.5),pos().y));
    points.push_back(PVector((-ServerParam::PITCH_LENGTH*0.5)-ServerParam::instance().goalPostRadius(),pos().y));
    points.push_back(PVector(pos().x,(ServerParam::PITCH_WIDTH*0.5) + ServerParam::instance().goalPostRadius()));
    points.push_back(PVector(pos().x,(-ServerParam::PITCH_WIDTH*0.5) - ServerParam::instance().goalPostRadius()));

    PVector nearest_point = PVector(1000,1000);
    for (int i = 0; i < 4; i++)
    {
        if(nearest_point.distance(pos()) >= points.at(i).distance(pos()))
        {
            nearest_point = points.at(i);
        }
    }
    CArea post = CArea( nearest_point
    , ServerParam::instance().goalPostRadius() + M_size );
    ...
    while ( ( pos() - post.center() ).r() < post.radius() )
    {
        ...
        {
            /*if((post.center().x < pos().x) ||
                post.center().x > pos().x)
            {
                M_vel.x = - M_vel.x ;
            }
            if((post.center().y < pos().y) ||
                post.center().y > pos().y)
            {
                M_vel.y = - M_vel.y ;
            }*/
